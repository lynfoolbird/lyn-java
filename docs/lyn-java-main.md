...
三高思想：本地化(如引入二级本地缓存，微服务注册中心数据本地化)、池化技术(连接池对象池线程池等)、缓存缓冲（BufferMQ等）、异步、多线程并发并行、分布式(系统服务拆分、分库分表)、分流(负载均衡)、批处理（比如请求合并）、数据结构与算法（时间换空间，空间换时间等）、读写分离(mysql、redis)、动静分离
