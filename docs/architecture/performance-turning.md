...
11、代码层面（锁、事务控制、粒度上下文传递集合初始容量等，读多写少的场景用乐观锁 threadlocal、volatile+cas、disruptor；数据结构与算法（时间换空间，空间换时间等，贪心 分治 回溯 动态规划 查找排序 ）)，
...
高可用的方案主要从冗余（数据冗余（主从、多副本）、服务冗余（多实例避免单点））、取舍、系统运维（部署模式、监控告警、安全防护）、测试（灾备演练）3个方向考虑，同时需要有配套的值班机制和故障处理流程，当出现线上问题时，可及时跟进处理。

1、故障转移：对等节点的故障转移，Nginx和服务治理框架均支持一个节点失败后访问另一个节点。

​                        非对等节点故障转移，通过心跳检测并实施主备切换（比如redis哨兵模式、MySQL主从切换等）。

2、代码层面：超时重试、容错机制、幂等防重。数据一致性检查及修复。兜底策略、开关控制
...
9、部署模式：两地三中心架构、单元化架构、集群部署负载均衡弹性伸缩、灰度发布：能支持按机器维度进行小流量部署
