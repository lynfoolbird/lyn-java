...
本地思想（如引入二级本地缓存，注册中心数据本地化，远程调用落地，元数据本地化优先调用远程等)、

池化思想（内存池连接池对象池线程池等，预分配循环使用，复用)、

缓存缓冲（借助Buffer、MQ、本地缓存分布式缓存Map等，）、

异步思想（借助MQ，springevent、线程等，非核心逻辑异步化、Future、监听回调机制，非阻塞)、
...
分布式（集群、系统拆分，熔断限流降级、分流(负载均衡)、

数据结构与算法（时间换空间，空间换时间等，贪心 分治 回溯 动态规划 查找排序 KMP）、

分离思想（mysql、redis读写分离、动静分离CDN、冷热分离（不常访问历史数据转储归档单独访问或清理））

数据异构（合适的工具、关系型数据库，NoSQL、mongodb、es、mysql、hbase等)、

代码层面（锁、事务、粒度上下文传递集合初始容量等，threadlocal、volatile+cas、distrpor)，

存储层面（分库分表分区、sql优化索引锁事务、数据冗余字段冗余）

压缩传输（gzip等）

硬件底层（CPU内存网卡固态硬盘JVM；零拷贝、IO复用、无锁编程、多线程、缓存友好、高效序列化）

中间件层（组件调优：tomcat jvm db cache mq solr Nginx）
...
[CTO 偷传我的系统性能优化的十大绝招](https://mp.weixin.qq.com/s/ujDIauY9YbUWjto1IfruAQ)
...
1、分流思想：集群部署，通过负载均衡减轻单机压力。分布式

2、缓存缓冲：多级缓存，包括静态数据使用CDN、本地缓存、分布式缓存等，以及对缓存场景中的热点key、缓存穿透、缓存并发、数据一致性等问题的处理。使用缓存、MQ承接大流量。

3、分离思想：分库分表和索引优化，以及借助搜索引擎解决复杂查询问题。

4、数据异构：考虑NoSQL数据库的使用，比如HBase、Redis等，但是团队必须熟悉这些组件，且有较强的运维能力。考虑ES的使用

5、异步思想：将次要流程通过多线程、MQ、甚至延时任务进行异步处理。Future、回调、监听

6、流量控制：限流，需要先考虑业务是否允许限流（比如秒杀场景是允许的），包括前端限流、Nginx接入层的限流、服务端的限流。对流量进行削峰填谷，通过MQ承接流量。

7、本地思想：通过冗余数据的方式干掉RPC调用。

8、并发处理，通过多线程将串行逻辑并行化。

9、预取思想，比如抢红包场景，可以提前计算好红包金额缓存起来，发红包时直接使用即可。

​       缓存预热，通过异步任务提前预热数据到本地缓存或者分布式缓存中。

11、批量思想：减少IO次数，比如数据库和缓存的批量读写、RPC的批量接口支持。

12、压缩传输：减少IO时的数据包大小，包括采用轻量级的通信协议、合适的数据结构、去掉接口中的多余字段、减少缓存key的大小、压缩缓存value等。高性能序列化Probuf

13、代码层面：比如将大概率阻断执行流程的判断逻辑前置、For循环计算逻辑优化，或采用更高效的算法。

​        锁选择，读多写少的场景用乐观锁，或者考虑通过分段锁的方式减少锁冲突，无锁编程。事务控制

14、池化思想：包括HTTP请求池、线程池、数据库连接池、对象池和Redis连接池等。

15、硬件JVM：包括新生代和老年代的大小、GC算法的选择等，尽可能减少GC频率和耗时。顺序写、零拷贝
...
2、接口层面：超时设置、容错机制、重试策略和幂等设计。数据一致性检查及修复。兜底策略、开关控制
