...
无状态  不变类；线程私有threadlocal 栈上分配；volatile+cas自旋 无锁；加锁 synchronized lock；分布式锁
...
首先，让我们了解一下什么是并发队列。并发队列是在多线程环境下使用的数据结构，它可以安全地处理并发访问和操作。常见的并发队列类包括ArrayBlockingQueue、LinkedBlockingQueue和ConcurrentLinkedQueue等。

然而，当任务的产生速度超过了队列的处理速度时，队列可能会发生堆积，最终导致队列溢出。这种情况可能出现在以下几种场景中：

1、生产者速度过快： 如果生产者线程产生任务的速度非常快，而消费者线程处理任务的速度较慢，那么队列很容易堆积。这可能是由于生产者线程的处理逻辑复杂或阻塞等原因导致的。

2、消费者速度过慢： 如果消费者线程处理任务的速度较慢，无法及时释放队列中的任务，也会导致队列堆积。这可能是由于消费者线程的资源竞争、延迟加载或网络延迟等原因导致的。

那么，如何解决并发队列堆积导致队列溢出的问题呢？下面是一些常见的解决方案：

3、调整线程池配置： 当队列堆积时，可以通过调整线程池的核心线程数、最大线程数和队列容量等参数来适应不同的场景需求。增加线程数量和队列容量可以提高处理能力，但要谨防资源消耗过大。

4、使用有界队列： 将无界队列（例如ConcurrentLinkedQueue）替换为有界队列（例如ArrayBlockingQueue、LinkedBlockingQueue）可以限制队列的大小，避免堆积过多任务。

5、实现自定义策略： 可以实现自定义的拒绝策略来处理队列溢出。例如，使用ThreadPoolExecutor的
setRejectedExecutionHandler()方法，将任务添加到其他队列进行后续处理，或者丢弃部分任务等。


6、优化任务处理： 对于生产者线程，可以考虑优化任务生成的速度，尽量减少不必要的延迟。对于消费者线程，可以优化任务处理的逻辑，提高线程的执行效率。
