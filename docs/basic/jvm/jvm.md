...
**局部变量是线程安全的吗？**不一定，局部变量可能作为函数参数或返回值传递出去，可能成为别的对象的实例变量，被别的线程访问到，如过逃逸分析后在栈上分配或者局部变量不会逃逸出去则局部变量是线程安全的。

**对象一定分配在堆中吗？** 不一定的。

随着JIT编译器的发展与逃逸分析技术逐渐成熟，所有的对象都分配到堆上也渐渐变得不那么“绝对”了。其实，在编译期间，JIT会对代码做很多优化。其中有一部分优化的目的就是减少内存堆分配压力，其中一种重要的技术叫做逃逸分析。
...
-XX:+DoEscapeAnalysis ： 表示开启逃逸分析 从jdk 1.7开始已经默认开启逃逸分析
...
###  7.3.3 频繁 minor gc 怎么办？minorgc的触发时机？

新创建的对象优先在新生代 Eden 区分配，如果 Eden 区没有足够空间时，就会触发 Young GC 来清理理新生代。 

优化Minor GC频繁问题：通常情况下，由于新生代空间较小，Eden区很快被填满，就会导致频繁Minor  GC，因此可以通过增大新生代空间`-Xmn`来降低Minor GC的频率。

### 7.3.4 频繁Full GC怎么办？fullgc的触发时机？

**Young GC 之前检查老年代**：在要进行 Young GC 的时候，发现老年代可用的连续内存空间 < 新生代历次
Young GC后升入老年代的对象总和的平均大小 ，说明本次 Young GC 后可能升入老年代的对象大小，可能超过
老年代当前可用内存空间,那就会触发 Full GC。

**Young GC 之后老年代空间不足**：执行 Young GC 之后有一批对象需要放入老年代，此时老年代就是没有足
够的内存空间存放这些对象了，此时必须立即触发一次 Full GC

**老年代空间不足**：老年代内存使用率过高，达到一定比例，也会触发 Full GC。

**空间分配担保失败**：新生代的 To 区放不下从 Eden 和 From 拷贝过来对象，或者新生代对象 GC 年龄到达阈值需要晋升这两种情况，老年代如果放不下的话都会触发 Full GC。
...
### 7.3.5 内存泄漏和内存溢出问题？是如何定位的？

内存溢出：申请的内存超过了可用内存；内存泄漏：申请的内存没被正确释放导致内存被白白占用。

内存泄漏是导致内存溢出的原因之一。

内存泄漏的可能原因：连接或IO流未释放；hash值发生改变；ThreadLocal使用不当；变量作用域大于使用范围；长生命周期对象持有短生命周期对象的引用；
...
当**-Xms**和**-Xmx**设置成一样的值时，JVM就不需要对堆内存扩容，能够优化一些性能，但占用内存少时可能造成浪费
